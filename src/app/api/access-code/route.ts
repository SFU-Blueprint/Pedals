...
function handleDatabaseError(error: any): NextResponse {
  const errorMessage = error.message === 'TypeError: fetch failed'
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const token = jwt.sign({ accessCode: data.code }, JWT_SECRET, { expiresIn: '1h' });
    ...
    return NextResponse.json({ message: 'Access code validation successful.' }, { 
      status: 200,
      headers: {
