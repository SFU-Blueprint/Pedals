...
export async function POST(req: NextRequest){
  ...
  if (flag === "delete_users"){
    ...
    const errorShifts = await Promise.all(
      ids.map(async (id: string) => {
        ...
        if (errorShiftError) {
          return NextResponse.json(
            {
              message: "Error occurred while retrieving shifts. Please try again."
            },
            { status: 500 }
          );
        }
    ...
    const errorShiftVolunteerId = Array.from(new Set(
      errorShifts.flat().map((shift) => shift.volunteer_id)
    ));
    ...
    let errorShiftVolunteers = await Promise.all(
      errorShiftVolunteerId.map(async (id: string) => {
        ...
        if (errorShiftUserError) {
          ...
          return NextResponse.json(
            {
              message: "Error occurred while retrieving users with error shifts in deletion. Please try again."
            },
            { status: 500 }
          );
        }
    ...
    const activeShiftVolunteerId = Array.from(new Set(
      activeShifts.flat().map((shift) => shift.volunteer_id)
    ));
    ...
    let activeShiftVolunteers = await Promise.all(
      activeShiftVolunteerId.map(async (id: string) => {
        ...
        if (activeShiftUserError) {
          ...
          return NextResponse.json(
            {
              message: "Error occurred while retrieving users with active shifts in deletion. Please try again."
            },
            { status: 500 }
          );
        }
    ...
    if ((activeShiftVolunteers.length !== 0) || (errorShiftVolunteers.length !== 0)){
      return NextResponse.json(
        {data: [activeShiftVolunteers, errorShiftVolunteers], message: "User cannot be removed due to shift issues" },
        { status: 201 }
      );
    }
      return NextResponse.json(
